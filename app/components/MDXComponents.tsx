...
import type { DetailedHTMLProps, HTMLProps, HTMLAttributes, OlHTMLAttributes } from 'react'
import { highlight } from 'sugar-high'
import { FiCopy, FiCheck } from 'react-icons/fi'
import { 
  SiTypescript, SiJavascript, SiHtml5, SiCss3, SiPython, 
  SiRust, SiGo, SiOpenjdk, SiPhp, SiRuby, SiSwift, SiKotlin,
  SiCplusplus, SiC, SiCsharp, SiMarkdown, SiJson, SiYaml,
  SiReact, SiVuedotjs, SiAngular, SiSvelte, SiDart, 
  SiLua, SiPerl, SiHaskell, SiElixir, SiScala, SiClojure,
  SiPowershell, SiDocker, SiKubernetes, SiGnubash,
  SiGraphql, SiSass, SiLess, SiStyledcomponents, SiTailwindcss,
  SiPostgresql, SiMongodb, SiRedis, SiMysql, SiSqlite,
  SiVim, SiLatex, SiNixos, SiToml, SiApache, SiNginx,
  SiWebassembly, SiAssemblyscript, SiRescript
} from 'react-icons/si'
import { TbBrandReactNative } from 'react-icons/tb'

type PreProps = DetailedHTMLProps<HTMLAttributes<HTMLPreElement>, HTMLPreElement>
type CodeProps = DetailedHTMLProps<HTMLAttributes<HTMLElement>, HTMLElement>
type ImgProps = DetailedHTMLProps<HTMLAttributes<HTMLImageElement>, HTMLImageElement> & {
  src?: string;
  alt?: string;
}
...
export const components = {
        ...
        <div className="relative bg-[#090909] flex">
          ...
          >
            {Array.from({ length: lineCount }).map((_, i) => (
              <div 
                key={i} 
                className="text-right pr-2 leading-5 h-5"
              >
                {i + 1}
              </div>
            ))}
  ...
  code: ({ children, className, ...props }: CodeProps) => {
    if (!className) {
      return <code className="font-mono text-sm px-1.5 py-0.5" {...props}>{children}</code>
    }
  ...
  ol: ({ children, ...props }: DetailedHTMLProps<OlHTMLAttributes<HTMLOListElement>, HTMLOListElement>) => (
  ...
  blockquote: ({ children, ...props }: HTMLProps<HTMLQuoteElement>) => (
    <blockquote 
      className="border-l-4 border-blue-500 pl-6 my-8 italic text-gray-400" 
      {...props}
    >
      {children}
    </blockquote>
  ...
  p: ({ children, ...props }: HTMLProps<HTMLParagraphElement>) => (
    <p className="mb-6 text-gray-300 leading-relaxed" {...props}>{children}</p>
  ),
  YouTube: ({ 
    id,
    title,
